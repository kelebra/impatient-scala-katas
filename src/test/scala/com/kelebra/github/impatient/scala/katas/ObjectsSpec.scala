...
import com.kelebra.github.impatient.scala.katas.LengthUnitsImplicits.ViewToLengthUnit
...
class ObjectsSpec extends WordSpec with Matchers {
  ...
  "Inches" should {
    ...
    "be created with implicit conversion" in {
      (100 inches).value shouldBe 100
    }

    "convert to meters" in {
      (100.inches toMeters).value shouldBe 2.54
    }

    "convert to miles" in {
      (100.inches toMiles).value shouldBe 0.001578
    }
  }
  ...
  "Meters" should {
    ...
    "be created with implicit conversion" in {
      (100 meters).value shouldBe 100
    }

    "convert to inches" in {
      (100.meters toInches).value shouldBe 3937.00787400
    }

    "convert to miles" in {
      (100.meters toMiles).value shouldBe 0.06213700
    }
  }
  ...
  "Miles" should {
    ...
    "be created with implicit conversion" in {
      (100 miles).value shouldBe 100
    }

    "convert to inches" in {
      (100.miles toInches).value shouldBe 6.336e+6
    }

    "convert to meters" in {
      (100.miles toMeters).value shouldBe 160934.709
    }
  }
  ...
  "Inter conversion" should {
    ...
    "be transitive for meter through miles" in {
      (100 meters).toMiles.toMeters.value should be((100 meters).value +- Eps)
    }

    "be transitive for inches through meters" in {
      (100 inches).toMeters.toInches.value should be((100 inches).value +- Eps)
    }
  }
